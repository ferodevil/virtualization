...
3. 在虚机中配置合适的ip地址，并观察网络通路是否正常；  
4. 目前在4.19内核版本虚拟机发送的数据会被ipvlan丢弃，具体原因为ipvlan转发报文时没有找到wlan0地址，所以被直接丢弃；ipvlan模块初始化过程中会注册一个监听网卡事件的钩子函数，具体如下所示，当ipvtap虚拟网卡up后，正常情况下应该能收到wlan0的NETDEV_UP事件，而问题环境下未收到此事件，导致转发的时候找不到wlan0的地址；而且很奇怪的是在ipvtap创建后手动触发此钩子函数会造成kernel挂死。
```
